...
function populateHourly(hourlyData){
  for(var i=0;i<8;i++){
    $('.hourly-list').append("<li class='hourly-item'><p id='hourlyChanceOfRain'>Chance of rain "+Math.round(hourlyData[i].precipProbability*100)+"%</p><p id='hourlyHumidity'>Humidity "+Math.round(hourlyData[i].humidity*100)+"%</p><p id='hourlyTemp'>"+Math.round(hourlyData[i].temperature)+"&deg;</p><img id='hourlyIcon' src='img/"+hourlyData[i].icon+".png' /></li>");
  }
}

function populateDaily(hourlyData){
  for(var i=0;i<8;i++){
     $('.daily-list').append("<li class='daily-item'><p id='dailyChanceOfRain'>Chance of rain "+Math.round(hourlyData[i].precipProbability*100)+"%</p><p id='dailyHumidity'>Humidity "+Math.round(hourlyData[i].humidity*100)+"%</p><p id='dailyTemp'>"+Math.round(hourlyData[i].temperatureMax)+"&deg;</p><img id='dailyIcon' src='img/"+hourlyData[i].icon+".png' /></li>");
  }
}
